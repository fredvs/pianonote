...
uses
 ...
 mseifiglob, math, msestatfile, msestream, sysutils, msesimplewidgets,
 msedispwidgets,mserichstring, msebitmap;
...
noteName := trim(tstringedit1.value);
 ...
 if ((length(noteName) = 2) and (noteName[2] in ['0'..'9'])) or
     ...
     ((length(noteName) = 3) and (noteName[3] in ['0'..'9']) and (lowercase(noteName[2]) = 'b')) 
     then
    begin
  ...
  if err = 0 then octave := StrToInt(noteName[length(noteName)]);  { Last character is the octave number }
  
  { Calculate the semitone offset from A4, based on the note name and octave }
  if err = 0 then semitoneOffset := GetSemitoneOffset(notePart, octave);
   
  { Calculate the frequency of the note using the semitone offset }
  if err = 0 then noteFrequency := baseFrequency * exp(ln(root12of2) * semitoneOffset);
